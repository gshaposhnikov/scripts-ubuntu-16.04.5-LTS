...
echo $(tput setaf 2) "Would you like to configure as much as possible automatically? [Yes] yes" $(tput sgr 0)	# Копируйте эти команды в терминал.
...
# Root - настройки - персональные данные — персональные даные — Язык (Выбираем язык)
...
# Пользователю root на вкладке Общие права отмечаем всё выше перечисленное.
